<template>
  <div class="flex h-screen bg-gradient-to-br from-indigo-50 via-white to-orange-50 overflow-hidden">
    ...
    <div ref="leftSection" :style="{ backgroundImage: `url('/src/assets/image/branding.png')` }"
      ...
      <div class="relative w-full max-w-sm">
        <div class="rounded-2xl overflow-hidden shadow-lg">
          <img ref="heroImage" src="https://images.unsplash.com/photo-1552664730-d307ca884978?w=500&h=400&fit=crop"
            alt="Team collaboration" class="w-full h-80 object-cover" />
        </div>
      </div>
    ...
    <div ref="rightSection"
      ...
      <div class="w-full lg:max-w-md max-w-sm space-y-3">
        <!-- Back Button -->
        <button @click="goBack" class="lg:flex hidden items-center text-gray-600 hover:text-gray-900 transition mb-2">
          ...
          <div v-if="isLogin" class="space-y-4">
            ...
            <form @submit.prevent="handleLogin" class="space-y-3">
              <div>
                ...
                <input id="email" type="email" v-model="loginData.email" placeholder="john@example.com" required
              ...
              <div>
                ...
                <div class="relative">
                  <input id="password" :type="showLoginPassword ? 'text' : 'password'" v-model="loginData.password"
                    placeholder="Enter your password" required
                    class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition" />
                  <button type="button" @click="showLoginPassword = !showLoginPassword"
                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                    <svg v-if="!showLoginPassword" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5"
              ...
              <button type="button" @click="useDemoAccount" :disabled="isLoading"
          ...
          <div v-else class="space-y-4">
            ...
            <form @submit.prevent="handleRegister" class="space-y-2.5">
              <div class="grid grid-cols-2 gap-2 lg:gap-4">
                <div>
                  <label class="block text-xs lg:text-sm font-medium text-gray-700 mb-1 lg:mb-2">First name</label>
                  <input type="text" v-model="registerData.username" placeholder="John" required
                ...
                <div>
                  <label class="block text-xs lg:text-sm font-medium text-gray-700 mb-1 lg:mb-2">Last name</label>
                  <input type="text" v-model="registerData.Last_name" placeholder="Doe" required
                    class="w-full px-3 lg:px-4 py-2 bg-white lg:bg-gray-50 border border-gray-300 lg:rounded-lg rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition text-sm" />
                </div>
              </div>
              ...
              <div>
                <label class="block text-xs lg:text-sm font-medium text-gray-700 mb-1 lg:mb-2">Company name</label>
                <input type="text" v-model="registerData.company" placeholder="Your company name" required
                  class="w-full px-3 lg:px-4 py-2 bg-white lg:bg-gray-50 border border-gray-300 lg:rounded-lg rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition text-sm" />
              </div>

              <div>
                <label class="block text-xs lg:text-sm font-medium text-gray-700 mb-1 lg:mb-2">Phone (Optional)</label>
                <input type="tel" v-model="registerData.telephone" placeholder="+237 675 453 456"
                  class="w-full px-3 lg:px-4 py-2 bg-white lg:bg-gray-50 border border-gray-300 lg:rounded-lg rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition text-sm" />
              </div>

              <div>
                <label class="block text-xs lg:text-sm font-medium text-gray-700 mb-1 lg:mb-2">Password</label>
                <div class="relative">
                  <input :type="showRegisterPassword ? 'text' : 'password'" v-model="registerData.password"
                    placeholder="Create a password" required minlength="6"
                    class="w-full px-3 lg:px-4 py-2 bg-white lg:bg-gray-50 border border-gray-300 lg:rounded-lg rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition text-sm" />
                  <button type="button" @click="showRegisterPassword = !showRegisterPassword"
                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                    <svg v-if="!showRegisterPassword" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 lg:w-5 lg:h-5"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 lg:w-5 lg:h-5" viewBox="0 0 24 24"
              ...
              <div>
                <label class="block text-xs lg:text-sm font-medium text-gray-700 mb-1 lg:mb-2">Confirm password</label>
                <div class="relative">
                  ...
                  <button type="button" @click="showConfirmPassword = !showConfirmPassword"
                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                    <svg v-if="!showConfirmPassword" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 lg:w-5 lg:h-5"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="flex items-start space-x-2 pt-1">
                <input type="checkbox" v-model="registerData.accept_terms" required
