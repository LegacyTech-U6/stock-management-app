...
/>
  ...
  <div v-else-if="loading" class="flex justify-center items-center min-h-[400px] bg-white rounded-lg shadow-sm mt-6">
    ...
    <n-empty class="py-16 px-6" :description="search ? 'No customers found' : 'No customers yet'" size="large">
      ...
      <template #extra>
        <n-space vertical :size="12" align="center">
          <n-text depth="3">{{ search ? 'Try adjusting your search terms' : 'Get started by adding your first customer' }}</n-text>
          <n-button v-if="!search" type="success" @click="$emit('add')" size="large" class="bg-emerald-500 hover:bg-emerald-600 text-white">
            <template #icon><n-icon :component="Plus" /></template>
            Add Your First Customer
          </n-button>
          <n-button v-else @click="$emit('clear-search')" size="large" class="bg-gray-100 hover:bg-gray-200">
            Clear Search
          </n-button>
        </n-space>
      </template>
...
<script setup>
import { NCard, NDataTable, NPagination, NSelect, NSpin, NSpace, NEmpty, NButton, NIcon, NText } from 'naive-ui'
